...
export class CommandService {
  ...
  private isCommandAllowed(commandToExecute: string): boolean {
    ...
    // If allowedCommands is undefined, treat as empty array
    ...
    return new Promise((resolve, reject) => {
      ...
      process.stdout.on('data', (data) => {
        ...
        if (state) {
          ...
          // Check if waiting for input or dev server state
      ...
      // Timeout handling (set longer for multi-agent scenarios)
      setTimeout(() => {
        if (!isCompleted) {
          const state = this.processStates.get(pid)
          if (!state) return
